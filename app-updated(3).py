...
def cv_gas(flow: float, p1: float, p2: float, sg: float, t: float, k: float, 
    ...
    # Calculate unchoked Cv (always use Y=0.667 and x=x_actual)
    if x_actual > 0:
        unchoked_term = (sg * (t + C_TO_K) * z) / x_actual
        if unchoked_term > 0:
            unchoked_cv = (flow / (N7 * fp * p1 * 0.667)) * math.sqrt(unchoked_term)
        else:
            unchoked_cv = 0
    else:
        unchoked_cv = 0
    ...
    details = {
        ...
        'unchoked_cv': unchoked_cv,  # Add unchoked Cv to details
        'fp': fp,
        'expansion_factor': y,
        'is_choked': is_choked,
        'x_crit': x_crit,
            ...
            if recommended_valve:
                ...
                for i, scenario in enumerate(scenarios):
                    ...
                    with st.container():
                        ...
                        # Updated columns to include Unchoked Cv for gas flows
                        if scenario["fluid_type"] == "gas":
                            cols = st.columns([1.8, 1, 1, 1, 1, 1, 1, 1, 1.5])
                        else:
                            cols = st.columns([1.8, 1, 1, 1, 1, 1, 1, 1, 1.5])
                        ...
                        # Add Unchoked Cv column for gas flows
                        if scenario["fluid_type"] == "gas":
                            unchoked_cv = result['details'].get('unchoked_cv', 0)
                            cols[3].metric("Unchoked Cv", f"{unchoked_cv:.1f}")
                            cols[4].metric("Valve Cv", f"{actual_cv:.1f}")
                            cols[5].metric("Valve Size", f"{recommended_valve['valve'].size}\"")
                            cols[6].metric("Opening", f"{result['op_point']:.1f}%")
                            cols[7].metric("Margin", f"{result['margin']:.1f}%", 
                                        delta_color="inverse" if result['margin'] < 0 else "normal")
                            cols[8].markdown(f"**{warn_text}**")
                        else:
                            # For non-gas flows, shift columns to maintain layout
                            ...
                            cols[6].metric("Margin", f"{result['margin']:.1f}%", 
                                        delta_color="inverse" if result['margin'] < 0 else "normal")
                            cols[7].markdown("")  # Empty column for layout consistency
                            cols[8].markdown(f"**{warn_text}**")
            ...
            for i, scenario in enumerate(scenarios):
                ...
                with st.container():
                    ...
                    if scenario["fluid_type"] == "gas":
                        ...
                        cols[3].metric("Unchoked Cv", f"{unchoked_cv:.1f}")
                        cols[4].metric("Valve Cv", f"{actual_cv:.1f}")
                        cols[5].metric("Valve Size", f"{selected_valve.size}\"")
                        cols[6].metric("Opening", f"{result['op_point']:.1f}%")
                        cols[7].metric("Margin", f"{result['margin']:.1f}%", 
                                    delta_color="inverse" if result['margin'] < 0 else "normal")
                        cols[8].markdown(f"**{warn_text}**")
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        # Add Unchoked Cv display for gas flows
                        if scenario["fluid_type"] == "gas":
                            unchoked_cv_val = result['details'].get('unchoked_cv', 0)
                            if isinstance(unchoked_cv_val, (int, float)):
                                st.markdown(f"**Unchoked Cv (Y=0.667, X=X_actual):** {unchoked_cv_val:.1f}")
                            else:
                                st.markdown(f"**Unchoked Cv (Y=0.667, X=X_actual):** {unchoked_cv_val}")
