...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        ...
        prev_fluid_key = f"prev_fluid_library_{scenario_num}"
        if prev_fluid_key not in st.session_state:
            st.session_state[prev_fluid_key] = fluid_library
        
        # Check if fluid library has changed
        fluid_changed = st.session_state[prev_fluid_key] != fluid_library
        if fluid_changed:
            st.session_state[prev_fluid_key] = fluid_library
        ...
        if fluid_library != "Select Fluid Library...":
            ...
            # Always update fluid properties when fluid library is selected, regardless of type
            if fluid_changed or st.session_state.get(f"force_update_{scenario_num}", False):
                # Reset force update flag
                st.session_state[f"force_update_{scenario_num}"] = False
                
                # Update all fluid properties based on current temperature and pressure
                ...
                # Update thermal properties for gases and steam
    ...
    with col2:
        # Check if temperature or pressure has changed and force update if using fluid library
        prev_temp_key = f"prev_temp_{scenario_num}"
        prev_p1_key = f"prev_p1_{scenario_num}"
        
        if prev_temp_key not in st.session_state:
            st.session_state[prev_temp_key] = temp
        if prev_p1_key not in st.session_state:
            st.session_state[prev_p1_key] = p1
            
        temp_changed = st.session_state[prev_temp_key] != temp
        p1_changed = st.session_state[prev_p1_key] != p1
        
        if temp_changed or p1_changed:
            st.session_state[prev_temp_key] = temp
            st.session_state[prev_p1_key] = p1
            
            # Force update on next render if using fluid library
            if fluid_library != "Select Fluid Library...":
                st.session_state[f"force_update_{scenario_num}"] = True
                st.rerun()  # Trigger rerun to update properties
        
        if fluid_library != "Select Fluid Library...":
            # Properties are managed by the fluid library, inputs are disabled
            if fluid_type in ["liquid", "gas"]:
                sg = st.number_input(
                    ...
                    disabled=True  # Always disabled for library fluids
                )
            ...
            # Manual input mode - properties are editable
            if fluid_type in ["liquid", "gas"]:
                sg = st.number_input(
                    "Specific Gravity (water=1)" if fluid_type == "liquid" else "Specific Gravity (air=1)",
                    min_value=0.01, 
    ...
    # Update scenario_data with current values
