...
def calculate_valve_velocity(scenario, valve, op_point):
    ...
    if orifice_velocity > VELOCITY_LIMITS.get(scenario["fluid_type"], 10):
        orifice_warning = f"High orifice velocity ({orifice_velocity:.1f} m/s) for {scenario['fluid_type']}! (max {VELOCITY_LIMITS.get(scenario['fluid_type'], 10)} m/s)"
    ...
    return orifice_velocity, orifice_warning, inlet_velocity, inlet_warning
