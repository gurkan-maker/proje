...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        # Akışkan tipine göre filtrele
        fluid_options = ["Select Fluid Library..."]
        for fluid_name, fluid_data in FLUID_LIBRARY.items():
            if fluid_data["type"] == fluid_type:
                fluid_options.append(fluid_name)
        ...
        if fluid_library != "Select Fluid Library...":
            st.text_input("Fluid Type", value=fluid_type.capitalize(), disabled=True, 
                         key=f"fluid_type_text_{scenario_num}")
    ...
    with col2:
        # Akışkan özelliklerini hesapla butonu
        calculate_props = st.button("Calculate Fluid Properties", 
                                   key=f"calc_props_{scenario_num}",
                                   use_container_width=True)
        
        # Akışkan özelliklerini saklamak için container
        props_container = st.container()
        
        with props_container:
            # Fluid library seçilmişse özellikleri otomatik hesapla
            if fluid_library != "Select Fluid Library...":
                try:
                    fluid_data = FLUID_LIBRARY[fluid_library]
                    
                    # Sıcaklık ve basınca bağlı özellikleri hesapla
                    if calculate_props or True:  # Her zaman hesapla
    ...
    # Senaryo için hesaplama butonu
    st.markdown("---")
    calculate_scenario = st.button(
        f"Calculate Scenario {scenario_num}", 
        type="primary",
        key=f"calc_scenario_{scenario_num}",
        use_container_width=True
    )
    ...
    if calculate_scenario:
        ...
        errors = []
        if flow_value <= 0:
            errors.append("Flow rate must be greater than 0")
        if p1 <= p2:
        ...
        if errors:
            ...
            st.success(f"Scenario {scenario_num} parameters validated successfully!")
