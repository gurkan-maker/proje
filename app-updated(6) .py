...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    # Initialize session state for tracking fluid library changes
    fluid_library_key = f"fluid_library_{scenario_num}"
    if fluid_library_key not in st.session_state:
        st.session_state[fluid_library_key] = "Select Fluid Library..."
    ...
    with col1:
        ...
        # Check if fluid library has changed
        previous_fluid_library = st.session_state[fluid_library_key]
        fluid_library_changed = (fluid_library != previous_fluid_library)
        st.session_state[fluid_library_key] = fluid_library
        
        # Store current values before potential update
        current_sg = scenario_data["sg"]
        current_visc = scenario_data["visc"]
        current_pv = scenario_data["pv"]
        current_pc = scenario_data["pc"]
        current_k = scenario_data["k"]
        current_z = scenario_data["z"]
        current_rho = scenario_data["rho"]
        current_fluid_type = scenario_data["fluid_type"]
    ...
    with col2:
        if fluid_library != "Select Fluid Library...":
            ...
            # Always update when a fluid library is selected, not just on change
            # This ensures properties are always synchronized
            scenario_data["fluid_type"] = fluid_type
            
            # Update properties based on the selected fluid library
            try:
                # Update specific gravity if available
                if fluid_data.get("sg") is not None:
                    scenario_data["sg"] = fluid_data["sg"]
                
                # Update other properties using the fluid functions
                if fluid_data.get("visc_func") and fluid_type == "liquid":
                    scenario_data["visc"] = fluid_data["visc_func"](scenario_data["temp"], scenario_data["p1"])
                
                if fluid_data.get("k_func") and fluid_type in ["gas", "steam"]:
                    scenario_data["k"] = fluid_data["k_func"](scenario_data["temp"], scenario_data["p1"])
                
                if fluid_data.get("pv_func") and fluid_type == "liquid":
                    scenario_data["pv"] = fluid_data["pv_func"](scenario_data["temp"], scenario_data["p1"])
                ...
                if fluid_data.get("rho_func") and fluid_type == "steam":
                    scenario_data["rho"] = fluid_data["rho_func"](scenario_data["temp"], scenario_data["p1"])
                
                if fluid_data.get("z_func") and fluid_type == "gas":
                    scenario_data["z"] = fluid_data["z_func"](scenario_data["temp"], scenario_data["p1"])
            ...
            # When no fluid library is selected, allow manual selection
            try:
    ...
    with col2:
        # Update fluid properties when temperature or pressure changes, if using fluid library
        if fluid_library != "Select Fluid Library...":
            fluid_data = FLUID_LIBRARY[fluid_library]
            
            # Recalculate properties that depend on temperature/pressure
            try:
                if fluid_data.get("visc_func") and scenario_data["fluid_type"] == "liquid":
                    scenario_data["visc"] = fluid_data["visc_func"](temp, p1)
                
                if fluid_data.get("k_func") and scenario_data["fluid_type"] in ["gas", "steam"]:
                    scenario_data["k"] = fluid_data["k_func"](temp, p1)
                
                if fluid_data.get("pv_func") and scenario_data["fluid_type"] == "liquid":
                    scenario_data["pv"] = fluid_data["pv_func"](temp, p1)
                
                if fluid_data.get("rho_func") and scenario_data["fluid_type"] == "steam":
                    scenario_data["rho"] = fluid_data["rho_func"](temp, p1)
                
                if fluid_data.get("z_func") and scenario_data["fluid_type"] == "gas":
                    scenario_data["z"] = fluid_data["z_func"](temp, p1)
            except Exception as e:
                st.error(f"Error updating fluid properties: {str(e)}")
        
        # Display properties with appropriate disabling
        if scenario_data["fluid_type"] in ["liquid", "gas"]:
            ...
            if not sg_disabled:  # Only update if manually entered
                scenario_data["sg"] = sg
    ...
    scenario_data.update({
        "name": scenario_name,
        "fluid_type": scenario_data["fluid_type"],
        "flow": flow_value,
        "p1": p1,
        "p2": p2,
        "temp": temp,
