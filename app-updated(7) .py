...
def evaluate_valve_for_scenario(valve, scenario):
   ...
   # Calculate required Cv
    if scenario["fluid_type"] == "liquid":
        ...
        # FIXED: Consistent xT calculation for both passes
        def calculate_xt_for_opening(opening_percent):
            """Calculate xT for a given opening percentage"""
            xt_at_op = valve.get_xt_at_opening(opening_percent)
            if abs(pipe_d - valve_d) > 0.01:
                return calculate_x_tp(valve, valve_d, pipe_d, fp)
            return xt_at_op
        
        # First pass: calculate with Xt at 100% opening
        xt_initial = calculate_xt_for_opening(100)
        ...
        # FIXED: Second pass: recalculate with Xt at actual operating point
        xt_at_op = calculate_xt_for_opening(open_percent)
        cv_req, details = cv_gas(
            flow=scenario["flow"],
            p1=scenario["p1"],
            p2=scenario["p2"],
            ...
            t=scenario["temp"],
            k=scenario["k"],
            xt_at_op=xt_at_op,  # FIXED: Use the updated xT
            z=scenario["z"],
            fp=fp
        )
        ...
        # FIXED: Final xT calculation for the actual operating point
        final_xt = calculate_xt_for_opening(open_percent)
        # Update details with final xT values
        fk = scenario["k"] / 1.4
        x_crit = fk * final_xt
        x_actual = (scenario["p1"] - scenario["p2"]) / scenario["p1"]
        
        details['xt_at_op'] = final_xt
        details['x_crit'] = x_crit
        details['x_actual'] = x_actual
        ...
        # FIXED: Consistent xT calculation for both passes (same as gas)
        def calculate_xt_for_opening(opening_percent):
            """Calculate xT for a given opening percentage"""
            xt_at_op = valve.get_xt_at_opening(opening_percent)
            if abs(pipe_d - valve_d) > 0.01:
